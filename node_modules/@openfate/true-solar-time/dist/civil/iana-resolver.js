...
exports.resolveCivilTime = resolveCivilTime;
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
function getOffsetAtInstant(date, timeZoneId) {
    var _a, _b;
    const parts = new Intl.DateTimeFormat('en-US', {
        timeZone: timeZoneId,
        timeZoneName: 'longOffset'
    }).formatToParts(date);
    const str = (_b = (_a = parts.find(p => p.type === 'timeZoneName')) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : 'GMT+00:00';
...
// --------------------------------------------------------------------------
function computeOffsets(date, timeZoneId) {
...
// --------------------------------------------------------------------------
function formatLocalParts(date, timeZoneId) {
    const parts = new Intl.DateTimeFormat('en-US', {
        timeZone: timeZoneId,
...
// --------------------------------------------------------------------------
function resolveCivilTime(input) {
    const { year, month, day, hour, minute, second = 0, timeZoneId, timeZoneOffset, dstOffset = 0, disambiguation = 'compatible' } = input;
    // ------------------------------------------------------------------
    ...
    if (!timeZoneId)
        throw new Error('Must provide either timeZoneId or timeZoneOffset.');
    // ------------------------------------------------------------------
    ...
    for (let offsetMinutes = -14 * 60; offsetMinutes <= 14 * 60; offsetMinutes++) {
        const probeDate = new Date(baseMs - offsetMinutes * 60000);
    ...
    if (unique.length === 0) {
        ...
        const preBoundary = new Date(baseMs - 60 * 60000); // 1 h before
        const gapBoundaryOffset = getOffsetAtInstant(preBoundary, timeZoneId);
        selectedUtc = new Date(baseMs - gapBoundaryOffset * 60000);
    }
    ...
    return Object.assign({ calendarYear: local.year, calendarMonth: local.month, calendarDay: local.day, calendarHour: local.hour, calendarMinute: local.minute, utcTimestamp: selectedUtc.getTime() }, offsets);
}
